...
Dieses Kapitel führt die Konzepte ein, mit denen die Failure Cases in
Kapitel~\ref{sec:cases} eingeordnet werden.
...
Für diese Fallstudie sind zwei Verhaltensklassen bei Prozessausfällen relevant
\cite{cristian1991}. Bei einem
...
entstehen. Charakteristisch ist die lange Zeitspanne zwischen Ursache und sichtbarem
Symptom; im 24/7-Betrieb kann diese Stunden oder ganze Schichten betragen. Der Zustand
ist besonders problematisch, wenn das System nach außen betriebsbereit wirkt, seinen
eigentlichen Zweck aber nicht mehr erfüllt \cite{birman2012}.
...
als die ursprüngliche Schwachstelle liegt. Case~1 (Abschnitt~\ref{sec:case1}) zeigt
Fail-Silent-Verhalten empirisch, Case~3 (Abschnitt~\ref{sec:case3}) einen Kaskadeneffekt.
...
vor, der den Laufzeitstatus aller untergeordneten Prozesse kontinuierlich überwacht und
bei Ausfall automatisch reagiert -- durch Neustart, Alarm oder geordnetes Herunterfahren.
Dieses Konzept ist im Erlang/OTP-Framework als \textit{Supervision Tree} etabliert: Dort
überwachen Supervisoren ihre Kindprozesse und starten sie bei Absturz neu
...
und industrielle Systeme sind mehrere Varianten dieses Musters mit unterschiedlichen
Reaktionsstrategien dokumentiert \cite{pont_watchdog}.

Fehlt eine solche Logik, entstehen zwei Ausfallbilder. Im ersten hält ein
...
Bereitschaftsdienst nicht zeitnah erfolgen. Case~1 (Abschnitt~\ref{sec:case1}) quantifiziert
beide Ausfallbilder für ROSI.
...
Mechanismus. Ein blockierendes \texttt{put()} ohne Timeout überträgt den Stau auf alle
vorgelagerten Komponenten \cite{kleppmann2017}. In einem Multiprocessing-System
...
\textit{Single Point of Failure} für alle abhängigen Komponenten. Case~2
(Abschnitt~\ref{sec:case2}) untersucht diesen Mechanismus für ROSI, Case~3
(Abschnitt~\ref{sec:case3}) die blockierende Queue.
...
unkontrollierten Absturz. Case~4 (Abschnitt~\ref{sec:case4}) prüft dieses Muster für die
Datenbankverbindung von ROSI.
