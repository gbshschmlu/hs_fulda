...
Inter-Prozess-Kommunikation zu blockieren. Der Startprozess main.py teilt das
...
InspectionDataHandler erzeugt fortlaufend leere Daten-Container und schiebt sie
...
FrameGrabber bindet die Kamerahardware an, schreibt die rohen Pixeldaten in
Shared-Memory-Blöcke und reicht Referenzen darauf an den PipelineManager weiter.
Dieser orchestriert die eigentliche Verarbeitung. Der LoggerProcess nimmt die
...
Eine Übersicht aller Komponentennamen findet sich im Glossar auf
Seite~\pageref{sec:glossar}.

Für die Fehlertoleranz ist eine Eigenschaft dieser Topologie zentral:
main.py überwacht zur Laufzeit ausschließlich den PipelineManager
...
\begin{figure}[htbp]
    ...
    \caption[Prozess-Topologie von ROSI]{Prozess-Topologie von ROSI.
        main.py überwacht zur Laufzeit nur den PipelineManager
...
Der PipelineManager führt jede Inspektion durch eine streng sequenzielle
Pipeline aus sechs Schritten. Rechenintensive Schritte -- KI-Inferenz, Geometrievermessung
und Bildkompression -- lagert er über ein Map-Reduce-Muster in den MPWorkerPool
...
Der ResultCollector sammelt die Teilergebnisse nicht blockierend ein und
benachrichtigt die Pipeline erst, wenn alle Teiljobs abgeschlossen sind; ein
HealthMonitor überwacht die Heartbeats der Worker und startet einen nicht mehr
