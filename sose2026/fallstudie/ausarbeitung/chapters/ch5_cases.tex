...
\subsection[Case 1 -- Lücken in der Laufzeit-Prozesssupervision]{Case 1 -- Lücken in der Laufzeit-Prozesssupervision (FF2)}
\label{sec:case1}

Wie in Abschnitt~\ref{sec:architektur-topologie} beschrieben, überwacht das System zur
Laufzeit ausschließlich den PipelineManager; eine Supervisor-Logik im Sinne von
Abschnitt~\ref{sec:grundlagen-supervision} existiert für die drei übrigen Service-Prozesse
nicht. Um die Auswirkung dieser Lücke zu quantifizieren, wurde jeder Service-Prozess
einzeln per \texttt{SIGKILL} zum Absturz gebracht. Tabelle~\ref{tab:case1} zeigt, ob der
Hauptprozess den Ausfall erkennt und welche Systemreaktion daraus folgt.
...
\subsection[Case 2 -- Flüchtiger Bildspeicher als Single Point of Failure]{Case 2 -- Flüchtiger Bildspeicher als Single Point of Failure (FF1)}
\label{sec:case2}
...
\subsection[Case 3 -- Log-Queue-Stau als Pipeline-Blocker]{Case 3 -- Log-Queue-Stau als Pipeline-Blocker (FF3)}
\label{sec:case3}
...
\subsection[Case 4 -- Veraltete Datenbankverbindung nach Produktionspause]{Case 4 -- Veraltete Datenbankverbindung nach Produktionspause (FF3)}
\label{sec:case4}
...
\subsection[Case 5 -- Vermutete Race Condition im Speichermanagement]{Case 5 -- Vermutete Race Condition im Speichermanagement (FF1)}
\label{sec:case5}

Der in Abschnitt~\ref{sec:architektur-speicher} beschriebene Garbage Collector zerstört
Shared-Memory-Blöcke, auf die mehr als zehn Sekunden nicht zugegriffen wurde
\cite{python_shm}. Die Ausgangshypothese
